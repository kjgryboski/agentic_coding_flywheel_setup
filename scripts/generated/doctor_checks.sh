...
declare -a MANIFEST_CHECKS=(
    ...
    "stack.beads_rust.1	beads_rust (br) - Rust issue tracker with graph-aware dependencies	br --version | grep -Eq '(^|[[:space:]])v?0[.]5[.]3([[:space:]]|\$)'	required	target_user"
    ...
    "stack.beads_viewer	bv TUI for Beads tasks	bv --version | grep -Eq '(^|[[:space:]])v?0[.]22[.]0([[:space:]]|\$)'	required	target_user"
