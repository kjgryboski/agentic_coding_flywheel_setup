...
declare -a MANIFEST_CHECKS=(
    ...
    "stack.meta_skill.1	Local-first knowledge management with hybrid semantic search (ms)	test \"\$(ms --version 2>/dev/null)\" = \"ms 0.2.2\"	required	target_user"
    ...
    "stack.jeffreysprompts.1	Curated battle-tested prompts for AI agents - browse and install as skills (jfp)	test \"\$(jfp --version 2>/dev/null)\" = \"jfp 0.1.0\"	optional	target_user"
    ...
    "stack.beads_rust.1	beads_rust (br) - Rust issue tracker with graph-aware dependencies	\"\$TARGET_HOME/.local/bin/br\" --version | grep -Eq '(^|[[:space:]])v?0[.]5[.]3([[:space:]]|\$)'	required	target_user"
    "stack.beads_rust.2	beads_rust (br) - Rust issue tracker with graph-aware dependencies	\"\$TARGET_HOME/.local/bin/br\" list --json 2>/dev/null	optional	target_user"
    "stack.beads_viewer	bv TUI for Beads tasks	\"\$TARGET_HOME/.local/bin/bv\" --version | grep -Eq '(^|[[:space:]])v?0[.]22[.]0([[:space:]]|\$)'	required	target_user"
    ...
    "stack.rch.1	Remote Compilation Helper - transparent build offloading for AI coding agents	test \"\$(rch --version 2>/dev/null)\" = \"rch 1.0.60 (commit 0a982fdee2ca)\"	optional	target_user"
    "stack.rch.2	Remote Compilation Helper - transparent build offloading for AI coding agents	test \"\$(rchd --version 2>/dev/null)\" = \"rchd 1.0.60 (commit 0a982fdee2ca)\"	optional	target_user"
    "stack.rch.3	Remote Compilation Helper - transparent build offloading for AI coding agents	test \"\$(rch-wkr --version 2>/dev/null)\" = \"rch-wkr 1.0.60 (commit 0a982fdee2ca)\"	optional	target_user"
    ...
    "stack.srps.1	System Resource Protection Script - ananicy-cpp rules + TUI monitor for responsive dev workstations	test -x /usr/local/bin/sysmoni	optional	target_user"
    ...
    "stack.eidetic_engine_cli	Durable, local-first, explainable memory for coding agents (ee)	test \"\$(ee --version 2>/dev/null)\" = \"ee 0.14.2\"	optional	target_user"
    "stack.franken_markdown	Pure-Rust Markdown engine rendering self-contained HTML and tagged PDF (fmd)	test \"\$(fmd --version 2>/dev/null)\" = \"fmd 0.4.2\"	optional	target_user"
