...
function generateVerifiedInstallerSnippet(
  ...
  if (tool === 'ms') {
    lines.push(
      ...
      '    fi',
      'else',
      `    log_error "${escapeBash(module.id)}: exact source commissioning is supported only on Linux"`,
    ...
    lines.push(
      ...
      `        log_error "${escapeBash(module.id)}: failed to prepare the confined source-build and cache directories"`,
      '        ACFS_LAST_MODULE_FAILURE_REASON="environment setup"',
      ...
      `        log_error "${escapeBash(module.id)}: source-build or exact cache directory is not a confined real directory"`,
      '        ACFS_LAST_MODULE_FAILURE_REASON="environment setup"',
